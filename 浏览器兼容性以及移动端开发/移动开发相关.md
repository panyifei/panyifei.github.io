...
### react在app内的浏览器的渲染问题
在chrome，safari下都是好的，但是手机app内打开会有极端的渲染问题。出现在初次没有被渲染，在后续update插入文档流时会有渲染问题。后来发现是这种情况下z-index会乱。浏览器处理z-index没有能按照默认的渲染。解决办法就是手动处理一下z-index。

### ios针对div点击事件失效
这种情况下设置一下`cursor:pointer`就好了，也是个兼容的bug
